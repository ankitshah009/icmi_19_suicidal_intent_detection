...
training_features,testing_features,training_target,testing_target=train_test_split(verbal_only,Y_labels,train_size=0.75,test_size=0.25,random_state=None)
#tpot_data = pd.read_csv('PATH/TO/DATA/FILE', sep='COLUMN_SEPARATOR', dtype=np.float64)
#features = tpot_data.drop('target', axis=1).values
#training_features, testing_features, training_target, testing_target = \
#            train_test_split(features, tpot_data['target'].values, random_state=None)

# Average CV score on the training set was:0.778571428571
exported_pipeline = GradientBoostingClassifier(learning_rate=0.1, max_depth=8, max_features=0.3, min_samples_leaf=11, min_samples_split=2, n_estimators=100, subsample=0.85)
...
### Best pipeline: GradientBoostingClassifier(input_matrix, learning_rate=0.1, max_depth=8, max_features=0.3, min_samples_leaf=11, min_samples_split=2, n_estimators=100, subsample=0.8500000000000001)
### 0.6521739130434783
